...
"""Requesty API client wrapper with tool integration."""
...
class RequestyClient(BaseLLMClient):
    """Requesty client wrapper with tool schema generation using OpenAI-compatible endpoints."""

    def __init__(self, model_parameters: ModelParameters):
        super().__init__(model_parameters)

        if self.api_key == "":
            self.api_key: str = os.getenv("REQUESTY_API_KEY", "")

        if self.api_key == "":
            raise ValueError("Requesty API key not provided. Set REQUESTY_API_KEY in environment variables or config file.")

        # Get optional headers from environment variables
        ...
        # Requesty uses OpenAI-compatible endpoints
    ...
    def chat(self, messages: list[LLMMessage], model_parameters: ModelParameters, tools: list[Tool] | None = None, reuse_history: bool = True) -> LLMResponse:
        """Send chat messages to Requesty with optional tool support."""
        ...
        if response is None:
            raise ValueError(f"Failed to get response from Requesty after max retries: {error_message}")
        ...
        if response.usage:
            usage = LLMUsage(
                ...
                cache_read_input_tokens=0,  # Requesty doesn't provide cache info
                reasoning_tokens=0  # Requesty doesn't provide reasoning tokens
            )
    ...
    def supports_tool_calling(self, model_parameters: ModelParameters) -> bool:
        ...
        # Most modern models on Requesty support tool calling
        # This is a conservative list - you can expand it based on Requesty's documentation
        tool_capable_models = [
